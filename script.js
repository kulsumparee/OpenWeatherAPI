...
function weatherUiData(weatherData) {
    ...
    let temprature = `${Math.round(weatherData.main.temp) - 32 }`
    document.querySelector('.deg').innerText = `${Math.floor(temprature*5/9)}°C`
    document.getElementById("Date").innerText = new Date().toDateString();
    let tempratureFeel = `${(weatherData.main.feels_like) - 32}`
    document.getElementById("dgree").innerHTML = `Feels Like:${Math.floor(tempratureFeel*5/9)}°C`
